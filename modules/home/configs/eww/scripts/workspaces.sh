...
get_app_icon() {
    case "$1" in
        ...
        *office*|libreoffice*) echo "" ;;
        ...
        steam|Steam) echo "󰓓" ;;
        *pdf*|okular|evince|zathura) echo "" ;;
...
get_workspaces() {
    ...
    for ws_id in $(seq $start_ws $end_ws); do
        ...
        # Get apps on this workspace and build display string
        local display_text="$ws_id"
        if [ "$occupied" = "true" ]; then
            # Get app data with class, initialTitle for clean names
            local apps_data=$(echo "$all_clients" | jq -c --argjson wsid "$ws_id" \
                'map(select(.workspace.id == $wsid)) | map({class: .class, name: .initialTitle})')
            
            # Count occurrences of each app class
            ...
            # Build display string with apps
            for app_class in "${!app_counts[@]}"; do
                ...
                # Format with brackets: "[icon name]" or "[icon name]]" for stacked
                if [ "$count" -gt 1 ]; then
                    display_text="${display_text}  [${icon} ${app_name}]]"
                else
                    display_text="${display_text}  [${icon} ${app_name}]"
                fi
            done
        ...
        json_output="${json_output}{\"id\": $ws_id, \"active\": $is_active, \"occupied\": $occupied, \"display\": \"$display_text\"}"
